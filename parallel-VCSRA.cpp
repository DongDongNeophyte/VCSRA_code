    ...
    for(i=0;i<len;i++){
        for(j=i+1;j<len;j++){
            ...
            for(n=0;n<leng;n++){
                    ...
                    else if(aligned[i][n]!='-'&& aligned[j][n]=='-'){
                       // printf("i %c j %c\n", aligned[i][n],aligned[j][n]);
                        tmp_score=tmp_score-1;
                    }
                    else if(aligned[i][n]!='-'&& aligned[j][n]!='-'&& aligned[i][n]!=aligned[j][n]){
                       // printf("i %c j %c\n", aligned[i][n],aligned[j][n]);
                        tmp_score=tmp_score-1;
                    }
                    else if(aligned[i][n]!='-'&& aligned[j][n]!='-'&& aligned[i][n]==aligned[j][n]){
...
string select_center_vec(vector<string> alignment,int *index){
    int sum1[672];
    int sum2[672];
    int sum3[672];
    int sum4[672];
    ...
    for(int i=1;i<alignment.size();i++){
        if(len>alignment[i].length()){
            len=alignment[i].length();
            center=alignment[i];
        }
    }
    printf("len is %d\n", len);
    ...
    for(int j=0;j<alignment.size();j++){
        ave1=ave1+sum1[j];ave2=ave2+sum2[j];
        ave3=ave3+sum3[j];ave4=ave4+sum4[j];
    }
    ave1=ave1/alignment.size()+0.5;ave2=ave2/alignment.size()+0.5;
    ave3=ave3/alignment.size()+0.5;ave4=ave4/alignment.size()+0.5;

    for(int j=0;j<alignment.size();j++){
       c[j]=(sum1[j]*ave1+sum2[j]*ave2+sum3[j]*ave3+sum4[j]*ave4)/(sqrt(sum1[j]*sum1[j]+sum2[j]*sum2[j]+sum3[j]*sum3[j]+sum4[j]*sum4[j])*sqrt(ave1*ave1+ave2*ave2+ave3*ave3+ave4*ave4));
    }
    ...
    *index=0;
    ...
    printf("index is %d\n",*index );
    printf("center length is %d\n",alignment[*index].length() );
    return center;
}
...
vector<vector<string> > mutiple_alignment(vector<vector<string> > aligned_all,vector<string> alignment){
    ...
    aligned_all.push_back(center_set);
    aligned_all.push_back(other_set);
    return aligned_all;;
}
...
int main(int argc, char** argv){
    ...
    if(my_rank==0){
     ...
     int count=0;
     for (vector<string>::iterator i=alignment.begin();i!=alignment.end();i++){
      for (int j=0;j<(*i).length();j++){
        if(char((*i)[j])==13){
          *i=(*i).substr(0,j)+(*i).substr(j+1,-1);
          //printf("%s\n","not a print char!");
          count++;
        }
      }
     ...
     string center=select_center_vec(alignment,ind);
     alignment.erase(alignment.begin()+ *ind);

     vector<string> aligned_center;
     vector<string> aligned_other;
     vector<vector<string> > aligned_all;
     int number=1;
     int index;
     int len;
     start=clock();
     printf("np=%d\n",np);
     int over=alignment.size()/(np-1);

     while(number<(np-1)){
        printf("number=%d\n",number);
         MPI_Send(&over,1,MPI_INT,number,0,MPI_COMM_WORLD);
         len=center.length();
         printf("send 1\n");
         MPI_Send(&len,1,MPI_INT,number,0,MPI_COMM_WORLD);
         printf("send 2\n");
         MPI_Send(center.c_str(),len,MPI_CHAR,number,0,MPI_COMM_WORLD);
         printf("send 3\n");
         for (int i=0; i<=over-1; i++) {
             index=(number-1)*over+i;
             len=alignment[index].length();
             MPI_Send(&len,1,MPI_INT,number,0,MPI_COMM_WORLD);
             printf("send 4\n");
             MPI_Send(alignment[index].c_str(),len,MPI_CHAR,number,0,MPI_COMM_WORLD);
             printf("send 5\n");
         }
         printf("for over\n");
          number++;
     ...
     if(number==np-1){
         ...
         MPI_Send(center.c_str(),len,MPI_CHAR,number,0,MPI_COMM_WORLD);
         printf("send 3\n");
         for (int i=(np-2)*over;i<alignment.size(); i++) {
             len=alignment[i].length();
             MPI_Send(&len,1,MPI_INT,number,0,MPI_COMM_WORLD);
             printf("send 4\n");
             MPI_Send(alignment[i].c_str(),len,MPI_CHAR,number,0,MPI_COMM_WORLD);
             printf("send 5\n");
         }
         printf("for over\n");
     }
     ...
     int over1;
     while(number>=1){
      MPI_Recv(&over1,1,MPI_INT,number,0,MPI_COMM_WORLD,&status);
      for(int j=1;j<=over1;j++){
          MPI_Recv(&lennow,1,MPI_INT,number,0,MPI_COMM_WORLD,&status);
          MPI_Recv(centernowchar,lennow,MPI_CHAR,number,0,MPI_COMM_WORLD,&status);
          centernow=centernowchar;
          aligned_center.push_back(centernow);
          
          MPI_Recv(&lennow,1,MPI_INT,number,0,MPI_COMM_WORLD,&status);
          MPI_Recv(othernowchar,lennow,MPI_CHAR,number,0,MPI_COMM_WORLD,&status);
          othernow=othernowchar;
          aligned_other.push_back(othernow);
      }
     }
     long start1,end1;
     start1=clock();
     aligned=merge(aligned_center,aligned_other);
     end1=clock();
     ...
     return 0;
       ...
       string now;
       char *nowchar;
       vector<string> alignmentsub;
       int len;
       MPI_Recv(&len,1,MPI_INT,0,0,MPI_COMM_WORLD,&status);
       printf("recv2\n");
       MPI_Recv(centerchar,len,MPI_CHAR,0,0,MPI_COMM_WORLD,&status);
       center=centerchar;
       printf("recv3\n");
       alignmentsub.push_back(center);
       for(int i=1;i<=over;i++){
          MPI_Recv(&len,1,MPI_INT,0,0,MPI_COMM_WORLD,&status);
          printf("recv4\n");
          MPI_Recv(nowchar,len,MPI_CHAR,0,0,MPI_COMM_WORLD,&status);
          now=nowchar;
          printf("recv5\n");
          alignmentsub.push_back(now);
       }
       vector<vector<string> > aligned_all;
       aligned_all=mutiple_alignment(aligned_all,alignmentsub);
       int over1=aligned_all[0].size();
       MPI_Send(&over1,1,MPI_INT,0,0,MPI_COMM_WORLD);
       for(int j=1;j<=over1;j++){
           len=aligned_all[0][j].length();
           MPI_Send(&len,1,MPI_INT,0,0,MPI_COMM_WORLD);
           MPI_Send(aligned_all[0][j].c_str(),len,MPI_CHAR,0,0,MPI_COMM_WORLD);
           len=aligned_all[1][j].length();
           MPI_Send(&len,1,MPI_INT,0,0,MPI_COMM_WORLD);
           MPI_Send(aligned_all[1][j].c_str(),len,MPI_CHAR,0,0,MPI_COMM_WORLD);
